...
horizon = 2
case_file_path =
    joinpath(dirname(@__FILE__), "data", case_name, date, "h" * string(horizon))
...
for file in iter_files
    if occursin("input", file)
        mv(
            ...
            joinpath(case_file_path, "input", file),
            force = true,
        )
    end
end
...
for batch_id in batch_ids
    file_outs_names = [
        file for file in iter_files if occursin("output", file) && occursin(batch_id, file)
    ]
    ...
    if length(file_outs_names) == 1
        mv(
            file_outs[1],
            joinpath(case_file_path, "output", file_outs_names[1]),
            force = true,
        )
        continue
    end
