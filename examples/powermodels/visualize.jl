...
file_train = [
    joinpath(case_file_path_train, file) for
    file in iter_files_train if occursin("input", file)
]
file_test = [
    joinpath(case_file_path_test, file) for
    file in iter_files_test if occursin("input", file)
]
...
case_file_path_output_soc =
    joinpath(case_file_path, "output", string(network_formulation_soc))
iter_files_out_soc = readdir(case_file_path_output_soc)
iter_files_out_soc = filter(x -> occursin("arrow", x), iter_files_out_soc)
file_outs_soc = [
    joinpath(case_file_path_output_soc, file) for
    file in iter_files_out_soc if occursin("output", file)
]
...
ac_soc = innerjoin(
    output_data[!, [:id, :operational_cost]],
    output_data_soc[!, [:id, :operational_cost_soc]],
    on = :id,
    makeunique = true,
)

ac_soc.error =
    (ac_soc.operational_cost .- ac_soc.operational_cost_soc) ./ ac_soc.operational_cost *
    100
mean(ac_soc.error)
...
function total_load_vector(input_data; is_test = false)
    ...
    for j = 1:size(input_data, 1), i = 1:num_loads
...
# Load divergence
theta_train =
    [acos(cossim(nominal_loads, Vector(input_data_train[i, Not(:id)]))) for i = 2:10000] * 180 /
    pi
norm_sim_train =
    [norm(Vector(input_data_train[i, Not(:id)])) / norm_nominal_loads for i = 2:10000]

theta_test =
    [
        acos(cossim(nominal_loads, Vector(load_vector_test[i, Not([:id, :in_hull])]))) for
        i = 1:size(load_vector_test, 1)
    ] * 180 / pi
norm_sim_test = [
    norm(Vector(load_vector_test[i, Not([:id, :in_hull])])) / norm_nominal_loads for
    i = 1:size(load_vector_test, 1)
]
...
l = @layout [a b]
p1 = plot(
    scatter(theta_train, norm_sim_train, proj = :polar, label = "Train", color = :blue),
);
p2 =
    plot(scatter(theta_test, norm_sim_test, proj = :polar, label = "Test", color = :orange));
...
load_vector.norm_loads =
    [norm(Vector(load_vector[i, Not(:id)])) for i = 1:size(load_vector, 1)] ./ norm_nominal_loads
...
for i = 1:k
    ...
    in_convex_hull = inconvexhull(
        Matrix(
            vcat(instance_load_max, instance_load_min)[
                !,
                Not([:id, :norm_loads, :operational_cost]),
            ],
        ),
        Matrix(joined_data[!, Not([:id, :norm_loads, :operational_cost])]),
        Gurobi.Optimizer,
        silent = false,
        tol = 0.1, # close to convex hull
    )

    instances_in_convex_hulls[i] =
        sort(joined_data[in_convex_hull, [:operational_cost, :norm_loads]], :norm_loads)
end
...
plt = plot(
    ...
    title = "AC OPF - IEEE300",
);
for i = 1:k
    plot!(
        instances_in_convex_hulls[i][!, :norm_loads] * 100,
        instances_in_convex_hulls[i][!, :operational_cost],
        label = "Direction $i",
        color = :red,
        alpha = 0.4,
    )
