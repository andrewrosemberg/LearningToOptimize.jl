...
"""
function generate_dataset_pglib(data_dir::String, case_name::String; download_files::Bool=true, filetype::Type{T},
    num_p=10
    ...
    problem_iterator = ProblemIterator(collect(1:num_p), Dict(p => collect(1.0:num_p) .* original_load))
    ...
    recorder = Recorder{filetype}(file, primal_variables=[Symbol("0_pg[$i]") for i in 1:number_generators])
...
# Generate dataset
success_solves, number_generators = generate_dataset_pglib(data_dir, case_name; download_files=true, filetype=CSVFile, num_p=num_p)
