...
@sync @distributed for i = 1:num_batches
    ...
    inhull[idx_range] = inconvexhull(
        Matrix(train_table[!, Not(:id)]),
        Matrix(batch[!, Not(:id)]),
        Gurobi.Optimizer,
    )
...
if filetype === ArrowFile
    Arrow.write(
        joinpath(case_file_path_input, "train", case_name * "_train_input" * ".arrow"),
        train_table,
    )
    Arrow.write(
        joinpath(case_file_path_input, "test", case_name * "_test_input" * ".arrow"),
        test_table,
    )
else
    CSV.write(
        joinpath(case_file_path_input, "train", case_name * "_train_input" * ".csv"),
        train_table,
    )
    CSV.write(
        joinpath(case_file_path_input, "test", case_name * "_test_input" * ".csv"),
        test_table,
    )
end
